...
df.in <- t(df)/apply(t(df),1,sum)

df.out <- ccrepe(x=df.in, iterations=1000, min.subj=10)

output.file <- paste(args[[1]], ".species.score.txt", sep="");
write.table(df.out$obs.sim.score, output.file, sep="\t");

output.file <- paste(args[[1]], ".species.qvalue.txt", sep="");
write.table(df.out$q.values, output.file, sep="\t");

output.file <- paste(args[[1]], ".species.pvalue.txt", sep="");
